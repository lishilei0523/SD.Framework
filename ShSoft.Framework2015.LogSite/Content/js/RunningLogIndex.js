...
function loadList(queryParams) {
    //1.指定url
    $.globalParas.datagrid.url = '/RunningLog/List';
    //2.指定列
...
function watch() {
    ...
    $.globalParas.showComWindow('查看日志明细', '/RunningLog/Detail/' + selectedRows[0].Id, 1024, 600);
}
    ...
    if (selectedRows.length > 0) {
        $.messager.confirm('Warning', '确定要删除吗？', function (result) {
            if (result) {
                ...
                //发送至后台
                $.post('/RunningLog/DeleteSelected', { 'selectedIds': selectedIds.toString() }, function (jsonData) {
